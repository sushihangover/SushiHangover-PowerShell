...
function Get-WindowsLicenseStatus{
    <#
        ...
        .SYNOPSIS
            Get-WindowsLicensingStatus -- Check the License Status of Windows...
        .DESCRIPTION
            The original script is from 'The Scripting Guys' and I changed it to allow 
            local Windows 8 machines to be checked while still supporting VMs if this is run all 
            a Hyper-V Host
    ...
    #>
    Param (
        [Parameter(Mandatory=$false)][alias('l')][Switch]$LocalHostOnly
    )
    Try {
        if ($LocalHostOnly.IsPresent) {
            $cim = New-CimSession
        } else {
...
Export-ModuleMember -function Get-WindowsLicenseStatus
